  ...
  const handleJoinCommunity = async () => {
    try {
      ...
      const response = await fetch(
        `/api/communities/${communityData.id}/join`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(t("community_presentation.join_error"));
      }
        ...
        <div className="bg-white rounded-xl p-8 max-w-md w-full shadow-2xl text-center">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <h2 className="text-2xl font-bold text-gray-900">
              {t("community_presentation.welcome")}
            </h2>
            <p className="text-gray-600">
              {t("community_presentation.join_success")}
            </p>
            <p className="text-gray-500 text-sm">
              {t("community_presentation.loading_content")}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm">
                {t("community_presentation.created_by")}{" "}
                {communityData.creator.fullName}
              </p>
            </div>
          </div>
        ...
        <div className="p-8 overflow-y-auto flex-grow">
          ...
          {presentation.video_url && (
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">
                {t("community_presentation.video_presentation")}
              </h3>
              <div className="aspect-video bg-black rounded-xl overflow-hidden shadow-lg">
                <iframe
                  ...
                  title={t("community_presentation.community_presentation")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 shadow-sm">
              <h3 className="font-semibold mb-3 text-blue-900 flex items-center">
                ...
                {t("community_presentation.community_purpose")}
              </h3>
            ...
            <div className="bg-green-50 p-6 rounded-xl border border-green-100 shadow-sm">
              <h3 className="font-semibold mb-3 text-green-900 flex items-center">
                ...
                {t("community_presentation.code_of_conduct")}
              </h3>
          ...
          <div className="bg-amber-50 p-6 rounded-xl border border-amber-100 shadow-sm mb-8">
            <h3 className="font-semibold mb-3 text-amber-900 flex items-center">
              ...
              {t("community_presentation.disclaimer")}
            </h3>
        ...
        <div className="border-t border-gray-200 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 flex-shrink-0 bg-gray-50 rounded-b-xl">
          <label className="flex items-center space-x-3 cursor-pointer">
            ...
            <span className="text-gray-700">
              {t("community_presentation.accept_code_of_conduct")}
            </span>
          </label>
          ...
          <div className="flex space-x-4">
            ...
            <button
              ...
              className={`px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors shadow-md flex items-center justify-center ${
                !hasAcceptedTerms ? "cursor-not-allowed" : "cursor-pointer"
              }`}
            >
              {isJoining ? (
                <>
                  <Loader
                    ...
                    className="mr-2"
                  />
                  {t("community_presentation.processing")}
                </>
              ) : (
                t("community_presentation.join_community")
              )}
            </button>
          </div>
        </div>
