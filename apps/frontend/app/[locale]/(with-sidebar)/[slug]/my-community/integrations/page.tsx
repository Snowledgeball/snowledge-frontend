...
import { Button, Calendar, Card, Label, Popover, PopoverContent, PopoverTrigger, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Separator, Switch } from "@repo/ui";
    ...
    <section className="w-full flex flex-col gap-8">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-primary font-medium text-base">
          ...
          Configure which platforms to collect data from. Only public data is collected.
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {platforms.map(platform => {
          ...
          return (
            <Card key={platform.key} className="flex flex-col gap-4 p-6 border shadow-sm">
              <div className="flex items-center gap-3">
                <SocialIcon url={platform.url} bgColor={platform.color} style={{ height: 40, width: 40, filter: isDisabled ? 'grayscale(1) opacity(0.5)' : 'none' }} />
                <div>
                  <h2 className="text-lg font-semibold leading-tight">{platform.name}</h2>
                  <div className="text-xs text-muted-foreground">{platform.type.charAt(0).toUpperCase() + platform.type.slice(1)}</div>
                </div>
                <div className="ml-auto flex items-center gap-2">
                  <Switch checked={enabled[platform.key as keyof typeof enabled]} onCheckedChange={() => handleToggle(platform.key)} aria-label={`Enable ${platform.name}`} />
                </div>
              </div>
              <Separator />
              <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={val => setTimeRange(r => ({ ...r, [platform.key]: val }))}
                  >
                ...
                <div className="flex flex-col gap-2">
                  <Label>Date</Label>
                  <div className={`w-64 ${timeRange[platform.key as keyof typeof timeRange] !== 'custom' ? 'pointer-events-none opacity-60' : ''}`}>
                    {/* <DatePickerDemo
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              className={cn(
                                ...
                                !dates[platform.key as keyof typeof dates] && "text-muted-foreground"
                              )}
                            >
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {dates && dates[platform.key as keyof typeof dates] ? format(dates[platform.key as keyof typeof dates] || '', "PPP") : <span>Pick a date</span>}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0">
                            <Calendar
                              ...
                              onSelect={(d: any) => setDates((ds: any) => ({ ...ds, [platform.key]: d }))}
                              initialFocus
                            />
                          </PopoverContent>
                        </Popover>
                ...
                <div className="flex flex-col gap-2">
                  <Label>{platform.type.charAt(0).toUpperCase() + platform.type.slice(1)}</Label>
                  <div className="w-64">
                    <MultiSelect
                      options={platform.options}
                      // value={selected[platform.key as keyof typeof selected]}
                      onChange={vals => setSelected((s: any) => ({ ...s, [platform.key]: vals }))}
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  Last fetched: <span className="font-semibold text-foreground">{platform.lastFetched}</span>
                </div>
                <div className="flex flex-row items-center justify-between gap-2 mt-4">
                  ...
                  <Button
                    ...
                    disabled={
                      isCollecting ||
                      isDisabled ||
                      selected[platform.key as keyof typeof selected].length === 0 ||
                      (timeRange[platform.key as keyof typeof timeRange] === 'custom' && !dates[platform.key as keyof typeof dates])
                    }
                  >
