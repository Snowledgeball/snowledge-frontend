...
export function UpgradeBanner({
  ...
  const tUpgrade = useTranslations("upgrade_banner");
  ...
  return (
    <div
      className={cn(
        "container px-6 w-full flex items-center justify-center mx-auto",
        className
      )}
    >
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-[oklch(0.81_0.1729_163.24)]" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div className="relative flex h-[35px] items-center gap-1 rounded-[6px] border border-border pl-2.5 pr-1 text-sm bg-[oklch(0.81_0.1729_163.24)] text-[oklch(0.21_0.006_285.885)]">
            <button
              className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-[oklch(0.21_0.006_285.885)] underline decoration-[oklch(0.8_0.01_285.823)] hover:decoration-[oklch(0.141_0.005_285.823)] underline-offset-[5px] outline-none focus-visible:!shadow-primary"
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}
              onClick={onClick}
            >
              {tUpgrade("button_text")}
            </button>
            <div>
              {tUpgrade("description_start")}{" "}
              <b>{tUpgrade("description_cta")}</b> {tUpgrade("description_end")}
            </div>
            {onClose && (
              <button
                onClick={onClose}
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-[oklch(0.21_0.006_285.885)] hover:bg-white/10"
              >
                <X size={16} className="text-[oklch(0.21_0.006_285.885)]" />
              </button>
            )}
          </div>
        </motion.div>
