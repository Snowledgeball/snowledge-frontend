...
>();
const pendingEnrichmentsCache = new Map<
  string,
  { data: any[]; timestamp: number }
>();
const postsWithPendingEnrichmentsCache = new Map<
  string,
  { data: any[]; timestamp: number }
>();
...
export default function VotingSession({ communityId }: VotingSessionProps) {
  ...
  const [postsWithPendingEnrichments, setPostsWithPendingEnrichments] =
    useState<PendingPost[]>([]);
  ...
  const fetchPostsWithPendingEnrichments = useCallback(
    async (forceRefresh = false) => {
      try {
        ...
        const response = await fetch(
          `/api/communities/${memoizedCommunityId}/posts/with-pending-enrichments`,
          {
            headers: {
              "Cache-Control": "max-age=120", // Cache de 2 minutes
            },
        ...
        console.error(
          "Erreur lors de la récupération des posts avec enrichissements en attente:",
          error
        );
      }
    },
    ...
    <div className="bg-white rounded-lg shadow-sm p-6" id="voting-sessions">
        ...
        <div className="border-b border-gray-200 mb-6">
          <div className="flex space-x-8">
            ...
            <button
              ...
              onClick={() => handleTabChange("propositions")}
            >
              Propositions
            </button>
          </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div
            className={activeTab === "creation" ? "block" : "hidden"}
            id="creation-content"
          >
          ...
          <div
            className={activeTab === "enrichissement" ? "block" : "hidden"}
            id="enrichissement-content"
          >
            {postsWithPendingEnrichments.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg">
                <p className="text-gray-500">
                  Aucun post n'a d'enrichissement en attente de validation
                </p>
              </div>
            ) : (
          ...
          <div
            className={activeTab === "propositions" ? "block" : "hidden"}
            id="propositions-content"
          >
            <ProposalsVotingSession communityId={memoizedCommunityId} />
          </div>
        </>
      )}
    </div>
